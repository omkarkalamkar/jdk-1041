...
class TelescopeOff(AbstractTelescopeOnOff):
    ...
    def do(self):
        ...
        try:
            ...
            return self.generate_command_result(ResultCode.FAILED, f"Error in calling Telescope Off in TM CSP Master Leaf {self.tm_leaf_csp_master_adapter.dev_name}: {e}")

        try:
            self.tm_leaf_sdp_master_adapter.Off()
        except Exception as e:
            return self.generate_command_result(ResultCode.FAILED, f"Error in calling Telescope Off in TM SDP Master Leaf {self.tm_leaf_sdp_master_adapter.dev_name}: {e}")
