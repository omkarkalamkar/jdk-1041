"""
Executor for TelescopeOff command.

Tries every device command even when individual invocations fail,
preserving the original TelescopeOff behaviour of attempting all devices.
"""
...
class TelescopeOffExecutor(CommandExecutor):
    """
    ...
    def execute(self, context: CommandContext) -> None:
        ...
        after individual device failures so that all subsystems are
        attempted.
    ...
    def _record_and_notify_failure(
        ...
        original TelescopeOff behaviour where any device exception returned
        ...
        # "Error in calling Off command for dish devices" for dish failures.
        if "dish" in command.device_name.lower():
            if "Error in calling Off command for dish devices" not in message:
                message = (
